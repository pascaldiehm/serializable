...
- About runtime. Without nesting, the worst-case runtime is actually O(n²) - though this is only reachable if you tempered with the save file or the execution order of your `exposed` function. As long as the order of the items in your save file matches the order of execution of your `exposed` function, `deserialize` and `check` will run in O(n). If you include nesting it gets slightly more complicated, because the saved data is read by both the nester (or, more precise: all levels of nesters above) and the nested class. Usually that is a constant (or close enough) but in the worst case, most lines are read once per nesting level, making the worst case runtime O(n\*d) if d is the maximum nesting depth.
